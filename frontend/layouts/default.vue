<template>
  <v-app id="default" :dark="isDark">
    ...
    <v-app-bar :color="isDark ? 'default' : 'white'" :dark="isDark" elevation="8" app class="px-4" elevate-on-scroll>
      ...
      <NuxtLink v-if="!user.id" to="/login"
        class="mr-1 v-btn v-btn--is-elevated v-btn--has-bg theme--light v-size--small">Đăng nhập</NuxtLink>
      <NuxtLink v-if="!user.id" to="/register"
        class="mr-1 v-btn v-btn--is-elevated v-btn--has-bg theme--light v-size--small">Đăng ký</NuxtLink>
      <Purchase v-if="user.id" />
...
export default {
  name: 'LayoutDefault',
  components: { SeoHead, Menu, Purchase, ModalError },
  data() {
    return {
      modalPurchase: false,
