...
class Qrcode{
	public static function createQrCode($bank_title, $amount = 0, $description = '', $template = 'print') {
		// https://img.vietqr.io/image/<BANK_ID>-<ACCOUNT_NO>-<TEMPLATE>.png?amount=<AMOUNT>&addInfo=<DESCRIPTION>&accountName=<ACCOUNT_NAME>
        // $template: compact2 - compact - qr_only - print
		
        $bank_id = self::getBankId($bank_title['bank_name']);
        $account_no = $bank_title['bank_number'];
        $account_name = $bank_title['bank_user_name'];
        $qrcode = 'https://img.vietqr.io/image/'.$bank_id.'-'.$account_no.'-'.$template.'.png?amount='.$amount.'&addInfo='.$description.'&accountName='.$account_name;
		return $qrcode;
	}
	public static function getBankId($shortName) {
		$listBank = self::listBank();
		for ($i=0; $i < count($listBank); $i++) { 
			if (strtolower($listBank[$i]['shortName']) == strtolower($shortName)) {
				return $listBank[$i]['bin'];
			}
		}
		return false;
	}
	...
	/**
	 * Các ngân hàng được hỗ trợ tạo QR code
	 */
